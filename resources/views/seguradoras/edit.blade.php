...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h3 mb-1">Editar Seguradora</h1>
        <p class="text-muted mb-0">Altere as informações da seguradora <strong>{{ $seguradora->nome }}</strong></p>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('seguradoras.show', $seguradora) }}" class="btn btn-outline-info">
        ...
        <div class="row g-4">
            ...
            <div class="col-md-8">
                ...
                <input type="text" 
                       ...
                       id="nome" 
                       name="nome" 
                       value="{{ old('nome', $seguradora->nome) }}" 
            ...
            <div class="col-md-4">
                ...
                <input type="text" 
                       ...
                       id="site" 
                       name="site" 
                       value="{{ old('site', $seguradora->site_formatado) }}" 
            ...
            <div class="col-12">
                ...
                <textarea class="form-control @error('observacoes') is-invalid @enderror" 
                          ...
                          maxlength="2000">{{ old('observacoes', $seguradora->observacoes) }}</textarea>
                <div class="form-text">
                    <span id="charCount">{{ strlen(old('observacoes', $seguradora->observacoes ?? '')) }}</span>/2000 caracteres
                </div>
                @error('observacoes')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                                ...
                                <div class="card-body p-3">
                                    <div class="form-check">
                                        <input class="form-check-input" 
                                               ...
                                               {{ in_array($produto->id, old('produtos', $seguradora->produtos->pluck('id')->toArray())) ? 'checked' : '' }}>
                                    ...
                                    @if($seguradora->produtos->contains($produto->id))
                                        <div class="mt-2">
                                            <small class="text-success">
                                                <i class="bi bi-check-circle me-1"></i>
                                                Vinculado em {{ $seguradora->produtos->find($produto->id)->pivot->created_at->format('d/m/Y') }}
                                            </small>
                                        </div>
                                    @endif
                                </div>
        ...
        <div class="col-md-8">
            ...
            <ul class="list-unstyled">
                ...
                <li class="mb-2">
                    ...
                    Remover produtos pode limitar futuras cotações
                </li>
                <li class="mb-2">
                ...
                <li class="mb-2">
                    ...
                    Parcerias com corretoras são gerenciadas separadamente
                </li>
            </ul>
        </div>
        <div class="col-md-4">
            ...
            <div class="d-flex justify-content-between mb-2">
                <span>Produtos:</span>
                <span class="badge bg-success">{{ $seguradora->produtos->count() }}</span>
            </div>
            <div class="d-flex justify-content-between mb-2">
                <span>Corretoras:</span>
                <span class="badge bg-info">{{ $seguradora->corretoras->count() }}</span>
            </div>
            <div class="d-flex justify-content-between mb-2">
                <span>Cotações:</span>
                <span class="badge bg-warning">{{ $seguradora->cotacoes->count() }}</span>
            </div>
            <div class="d-flex justify-content-between">
                <span>Criada em:</span>
                <span class="text-muted">{{ $seguradora->created_at->format('d/m/Y') }}</span>
            </div>
        </div>
    </div>
</div>
...
const originalValues = {
    nome: {!! json_encode($seguradora->nome) !!},
    site: {!! json_encode($seguradora->site_formatado) !!},
    observacoes: {!! json_encode($seguradora->observacoes) !!},
    produtos: {!! json_encode($seguradora->produtos->pluck('id')->toArray()) !!}
};
...
function selectOriginal() {
    document.querySelectorAll('input[name="produtos[]"]').forEach(checkbox => {
        checkbox.checked = originalValues.produtos.includes(parseInt(checkbox.value));
    });
}
...
function resetForm() {
    if (confirm('Tem certeza que deseja restaurar os valores originais?')) {
        ...
        document.getElementById('site').value = originalValues.site || '';
        document.getElementById('observacoes').value = originalValues.observacoes || '';
        selectOriginal();
        
        // Atualizar contador
        document.getElementById('charCount').textContent = originalValues.observacoes ? originalValues.observacoes.length : 0;
    }
}
</script>
@endsection