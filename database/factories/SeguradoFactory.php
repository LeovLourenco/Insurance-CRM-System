...
{
    protected $model = Segurado::class;

    public function definition()
    {
        return [
            'nome' => $this->faker->name(),
            'documento' => $this->generateCpf(), // CPF único
            'telefone' => $this->faker->cellphone(true), // Celular BR
        ];
    }
    ...
    private function generateCnpj()
    {
        ...
        $sum = $n12*2+$n11*3+$n10*4+$n9*5+$n8*6+$n7*7+$n6*8+$n5*9+$n4*2+$n3*3+$n2*4+$n1*5;
        $d1 = $sum % 11 < 2 ? 0 : 11 - $sum % 11;

        $sum = $d1*2+$n12*3+$n11*4+$n10*5+$n9*6+$n8*7+$n7*8+$n6*9+$n5*2+$n4*3+$n3*4+$n2*5+$n1*6;
